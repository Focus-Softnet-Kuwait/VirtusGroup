...
{
    public class PVOmorfiaController : ApiController
    {
        ...
        [Authorize]
        [HttpPost]
        public IHttpActionResult PostPVOmorfia([FromBody] PVOmorfia jvo)
        {
            string clientIp = HttpContext.Current.Request.UserHostAddress;
            WriteLog.writeLog("Incoming Client Request from IP: " + clientIp + "" + Environment.NewLine + "Action:POOmorfia/PostPVOmorfia");
            ...
            try
            {
                ...
                objHeader.Add("PurchaseAC__Id", 264);                    // INVENTORY
                //objHeader.Add("VendorAC__Id", jvo.Header.VendorACName);
                ...
                lstHash.Add(objHash);
                objHashRequest.data = lstHash;
                objHashRequest.url = "http://" + ServerIp + "/Focus8API/Transactions/771/" + MyVNo + "/";
                objHashRequest.result = 1;
                string sContent = JsonConvert.SerializeObject(objHashRequest);
                using (var client = new WebClient())
                {
                    ...
                    var response = client.UploadString("http://" + ServerIp + "/Focus8API/Transactions/771", sContent);
